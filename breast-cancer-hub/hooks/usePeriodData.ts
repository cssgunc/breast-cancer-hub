...
export function removePeriod(p: Date){ //filter is not inplace
  const newPeriod = {
    date: p.getUTCDate(),
    month: p.getUTCMonth(),
    year: p.getUTCFullYear()
  }
  GLOBAL_PERIOD_DATA = GLOBAL_PERIOD_DATA.filter(p=>p.date != newPeriod.date || p.month != newPeriod.month || p.year != newPeriod.year)
  savePeriods()
}

export async function savePeriods(){
  if (Platform.OS === 'web') {
    await AsyncStorage.setItem(periodTimetampsKey, JSON.stringify(GLOBAL_PERIOD_DATA));
  }else{
    await SecureStore.setItemAsync(periodTimetampsKey, JSON.stringify(GLOBAL_PERIOD_DATA));
  }
}

export function isPeriodDay(value: Date){
  return GLOBAL_PERIOD_DATA.some(p=>p.date == value.getUTCDate() && p.month==value.getUTCMonth() && p.year == value.getUTCFullYear())
}
...
export function isCheckupDay(value: Date){
  ...
  const p = new Date(ts.year, ts.month, ts.date + 10)
  return p.getUTCDate() == value.getUTCDate() && p.getUTCMonth() == value.getUTCMonth() && p.getUTCFullYear() == value.getUTCFullYear()
}
